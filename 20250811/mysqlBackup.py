...
class MySQLBackupApp:
    ...
    def create_widgets(self):
        ...
        # mysqldump路径设置
        ttk.Label(backup_frame, text="mysqldump路径:").grid(row=2, column=0, sticky=tk.E, padx=5, pady=5)
        self.mysqldump_path_entry = ttk.Entry(backup_frame)
        self.mysqldump_path_entry.grid(row=2, column=1, sticky=tk.EW, padx=5, pady=5)
        self.mysqldump_browse_btn = ttk.Button(backup_frame, text="浏览...", command=self.browse_mysqldump)
        self.mysqldump_browse_btn.grid(row=2, column=2, sticky=tk.E, padx=5, pady=5)

        # 压缩和加密选项放在同一行
        self.compress_var = tk.IntVar(value=0)
        self.compress_cb = ttk.Checkbutton(backup_frame, text="压缩备份", variable=self.compress_var)
        self.compress_cb.grid(row=3, column=0, sticky=tk.W, padx=5, pady=5)

        self.encrypt_var = tk.IntVar(value=0)
        self.encrypt_cb = ttk.Checkbutton(backup_frame, text="加密备份", variable=self.encrypt_var)
        self.encrypt_cb.grid(row=3, column=1, sticky=tk.W, padx=5, pady=5)
        ...
        # 解密功能
        decrypt_frame = ttk.LabelFrame(main_frame, text="文件解密", padding="2 2 2 2")
        decrypt_frame.pack(fill=tk.X, pady=5)

        ttk.Label(decrypt_frame, text="加密文件:").grid(row=0, column=0, sticky=tk.E, padx=5, pady=5)
        self.decrypt_file_entry = ttk.Entry(decrypt_frame)
        self.decrypt_file_entry.grid(row=0, column=1, sticky=tk.EW, padx=5, pady=5)
        self.decrypt_file_entry.insert(0, "*.enc")
        self.decrypt_browse_btn = ttk.Button(decrypt_frame, text="浏览...", command=self.browse_decrypt_file)
        self.decrypt_browse_btn.grid(row=0, column=2, sticky=tk.E, padx=5, pady=5)

        ttk.Label(decrypt_frame, text="解密密码:").grid(row=1, column=0, sticky=tk.E, padx=5, pady=5)
        self.decrypt_password_entry = ttk.Entry(decrypt_frame, show="*")
        self.decrypt_password_entry.grid(row=1, column=1, sticky=tk.EW, padx=5, pady=5)
        self.decrypt_btn = ttk.Button(decrypt_frame, text="解密文件", command=self.start_decrypt)
        self.decrypt_btn.grid(row=1, column=2, padx=5, pady=5)

        ttk.Label(decrypt_frame, text="输出路径:").grid(row=2, column=0, sticky=tk.E, padx=5, pady=5)
        self.decrypt_output_entry = ttk.Entry(decrypt_frame)
        self.decrypt_output_entry.grid(row=2, column=1, sticky=tk.EW, padx=5, pady=5)
        self.decrypt_output_browse_btn = ttk.Button(decrypt_frame, text="浏览...", command=self.browse_decrypt_output)
        self.decrypt_output_browse_btn.grid(row=2, column=2, sticky=tk.E, padx=5, pady=5)
    ...
    def browse_mysqldump(self):
        ...
        if path:
            self.mysqldump_path_entry.delete(0, tk.END)
            self.mysqldump_path_entry.insert(0, path)
    ...
    def browse_decrypt_file(self):
        ...
        if path:
            ...
            self.decrypt_file_entry.insert(0, path)
            
            # 默认输出路径为原文件名去掉.enc后缀
            output_path = path[:-4] if path.endswith('.enc') else path + '_decrypted'
    ...
    def browse_decrypt_output(self):
        ...
        if path:
            self.decrypt_output_entry.delete(0, tk.END)
            self.decrypt_output_entry.insert(0, path)

    def find_mysqldump(self):
        """查找mysqldump路径"""
        # 首先尝试在默认位置查找
        possible_paths = [
            os.path.join(os.path.dirname(__file__), 'bin', 'mysqldump.exe'),
            os.path.join(os.path.dirname(__file__), 'bin', 'mysqldump'),
        ...
        # 检查系统PATH环境变量
        mysqldump_executable = 'mysqldump.exe' if os.name == 'nt' else 'mysqldump'
        path_dirs = os.environ.get('PATH', '').split(os.pathsep)
        for path_dir in path_dirs:
            path = os.path.join(path_dir, mysqldump_executable)
    ...
    def start_decrypt(self):
        """开始解密"""
        # 验证输入
        if not self.decrypt_file_entry.get():
            messagebox.showerror("错误", "请选择要解密的文件")
            return

        if not self.decrypt_password_entry.get():
            messagebox.showerror("错误", "请输入解密密码")
            return

        if not self.decrypt_output_entry.get():
            messagebox.showerror("错误", "请选择解密文件输出路径")
            return
        ...
        # 在新线程中执行解密
        decrypt_thread = threading.Thread(target=self.do_decrypt)
    ...
    def do_backup(self):
        ...
        try:
            ...
            mysqldump_path = self.mysqldump_path_entry.get()
            ...
            if not mysqldump_path:
                self.log("错误: 未指定mysqldump路径", error=True)
                messagebox.showerror("错误", "请指定mysqldump路径！")
                return

            if not os.path.exists(mysqldump_path):
                self.log(f"错误: 找不到mysqldump，请确保MySQL客户端工具已安装并在路径: {mysqldump_path}", error=True)
                messagebox.showerror("错误", f"找不到mysqldump，请确保MySQL客户端工具已安装并在路径: {mysqldump_path}")
                return
    ...
    def do_decrypt(self):
        ...
        try:
            # 获取解密参数
            encrypted_file = self.decrypt_file_entry.get()
            password = self.decrypt_password_entry.get()
            output_file = self.decrypt_output_entry.get()

            # 检查文件是否存在
            if not os.path.exists(encrypted_file):
                self.log(f"错误: 找不到文件 {encrypted_file}", error=True)
                messagebox.showerror("错误", f"找不到文件 {encrypted_file}")
                return

            # 执行解密
            self.decrypt_file(encrypted_file, password, output_file)
            
            self.log(f"文件解密完成: {output_file}")
            self.logger.info(f"文件解密完成: {output_file}")
            self.status_var.set("解密完成")
            messagebox.showinfo("成功", f"文件解密完成: {output_file}")

        except Exception as e:
            self.log(f"解密失败: {str(e)}", error=True)
    ...
    def decrypt_file(self, encrypted_file_path, password, output_file_path):
        """解密文件"""
        try:
            # 生成密钥
            key = hashlib.sha256(password.encode()).digest()
