...
class PrinterWorker(QThread):
    ...
    def run(self):
        try:
            ...
            self.log_message.emit("所有工作表打印完成")
                    
        except Exception as e:
            self.print_status.emit("打印失败")
            self.log_message.emit(f"打印任务失败: {str(e)}")
    
    def print_to_pdf(self, sheet_name):
        """打印到PDF文件"""
        try:
            ...
            self.log_message.emit(f"PDF将保存到: {pdf_filename}")
            
            # 执行打印到PDF（这会弹出保存对话框，除非使用特殊方法）
            # 为了自动保存，我们使用ExportAsFixedFormat方法
...
class ExcelPrinterApp(QMainWindow):
    ...
    def init_ui(self):
        self.setWindowTitle("Excel工作表打印工具 - PDF输出增强版")
