    ...
    def run(self):
        try:
            ...
            for i, sheet_name in enumerate(self.sheet_names):
                ...
                # 检查是否是PDF打印机
                is_pdf_printer = "Adobe PDF" in self.printer_name or "Microsoft Print to PDF" in self.printer_name
                
                if is_pdf_printer:
                    self.print_status.emit("正在导出")
                    self.log_message.emit(f"正在导出工作表: {sheet_name}")
                else:
                    self.print_status.emit("正在打印")
                    self.log_message.emit(f"正在打印工作表: {sheet_name}")
            ...
            if "Adobe PDF" in self.printer_name or "Microsoft Print to PDF" in self.printer_name:
                self.log_message.emit(f"所有工作表导出完成，共生成 {len(generated_files)} 个PDF文件")
                if generated_files:
                    self.log_message.emit("生成的文件列表:")
                    for file in generated_files:
                        self.log_message.emit(f"  - {file}")
            else:
                self.log_message.emit("所有工作表打印任务完成")
...
class ExcelPrinterApp(QMainWindow):
    ...
    def init_ui(self):
        self.setWindowTitle("Excel工作表处理工具")
    ...
    def on_printer_changed(self, printer_name):
        """当打印机选择改变时调用"""
        # 获取当前选中的打印机
        current_printer = self.printer_combo.currentText()
        
        # 检查是否选择了PDF打印机
        is_pdf_printer = "Adobe PDF" in current_printer or "Microsoft Print to PDF" in current_printer
        ...
        # 更新按钮文本
        if is_pdf_printer:
            self.print_selected_button.setText("导出选中")
            self.print_all_button.setText("导出全部")
        else:
            self.print_selected_button.setText("打印选中")
            self.print_all_button.setText("打印全部")
    ...
    def update_status(self, status):
        ...
        if status == "导出成功" or status == "打印成功":
            self.status_label.setStyleSheet("color: green; font-weight: bold;")
        elif status == "导出失败" or status == "打印失败":
            self.status_label.setStyleSheet("color: red; font-weight: bold;")
        elif status == "正在导出" or status == "正在打印":
    ...
    def print_finished(self):
        ...
        # 如果是PDF打印机，启用打开文件夹按钮
        is_pdf_printer = "Adobe PDF" in self.printer_combo.currentText() or "Microsoft Print to PDF" in self.printer_combo.currentText()
        if is_pdf_printer:
