...
cbj = 10.0  # 成本价
shuliang = 1000  # 持股数量
email_add = ['653113161@qq.com']  # 接收邮件地址
color_bg_fg = '#FFFFFF'  # 表格背景色
ts.set_token('b0088a0010b48f8a0a2c6216580943d847cc9c1d6cf591c84502e940')
...
def pd_ztjytime():
    ...
    d1 = datetime.datetime.strptime(datetime.datetime.now().strftime('%Y-%m-%d') + ' 11:30:01',
                                    '%Y-%m-%d %H:%M:%S')
    d2 = datetime.datetime.strptime(datetime.datetime.now().strftime('%Y-%m-%d') + ' 13:00:00',
                                    '%Y-%m-%d %H:%M:%S')
...
def do_programe(code):
    ...
    if pd_ztjytime() == False:  # 判断是否在暂停交易的时间范围内
        try:
            ...
            all_zdie = round((now_jiage - cbj) / cbj * 100, 2)  # 计算股票持有期间内总的涨跌幅
            now_shizhi = round(shuliang * now_jiage, 2)  # 计算股票现在的市值
            ...
            if (abs(now_zdie) >= 3 and abs(now_zdie) < 3.09) or (abs(now_zdie) >= 6 and abs(now_zdie) < 6.05) or (
                ...
                email_comment.append(
                    '<p><font size="2" color="#000000">根据设置参数，现将监控到' + name + '(' + str(code) + ')的证券价格异动消息汇报如下：</font></p>')
                email_comment.append(
...
def send_Email(Email_address, email_text):
    """发送邮件"""
    from_addr = 'your_email@qq.com'  # 发出电子邮件的地址
    password = 'your_password'  # 发出电子邮件的密码
