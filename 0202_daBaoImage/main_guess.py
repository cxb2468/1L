...
class myMain (QMainWindow,Ui_Form):
    ...
    def initRange(self):
         self.left =1
         self.right = self.guessRange

    @property
    def _randomRange(self):
        return random.choice(self.num)

    def randomNum(self):
        self.guessRange = self._randomRange

        self.guessNum = random.randint(1,self.guessRange)
        self.initRange()

    def intiUi(self):
        self.label_1.setText("数值范围是：{}-{}".format(self.left,self.right))
