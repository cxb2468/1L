...
from PyQt5.QtWidgets import (
    QApplication, QWidget, QVBoxLayout, QHBoxLayout, QLineEdit, QPushButton, QTableWidget, QTableWidgetItem, QMessageBox, QDialog, QFormLayout, QHeaderView, QCheckBox, QAbstractItemView, QLabel, QComboBox, QMenu, QFileDialog
)
...
class PasswordManager(QWidget):
    ...
    def initUI(self):
        ...
        # 导入按钮（蓝色）放在右上角
        import_button = QPushButton('导入', self)
        import_button.setStyleSheet('background-color: blue; color: white; font-weight: bold; font-size: 18px; padding: 10px;')
        import_button.clicked.connect(self.import_from_excel)
        top_layout.addWidget(import_button, alignment=Qt.AlignRight)

        # 导出按钮（橙色）放在右上角
        export_button = QPushButton('导出', self)
        export_button.setStyleSheet('background-color: orange; color: white; font-weight: bold; font-size: 18px; padding: 10px;')
        export_button.clicked.connect(self.export_to_excel)
        top_layout.addWidget(export_button, alignment=Qt.AlignRight)
    ...
    def show_add_dialog(self, record=None):
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            if website and username and password:
                if record:
                    # 编辑模式
                    self.execute_query("UPDATE passwords SET website = ?, username = ?, password = ?, notes = ? WHERE id = ?", 
                                       (website, username, password, notes, record[0]))
                else:
                    # 添加模式
                    existing_record = self.execute_query("SELECT id FROM passwords WHERE website = ?", (website,))
                    if existing_record:
                        QMessageBox.warning(self, '错误', '该网址已存在！')
                    else:
                        self.execute_query("INSERT INTO passwords (website, username, password, notes) VALUES (?, ?, ?, ?)", 
    ...
    def load_passwords(self, search_keyword=None):
        ...
        if search_keyword:
            # 全局搜索，不限制当前页
            passwords = self.execute_query(f"SELECT id, website, username, password, notes FROM passwords WHERE website LIKE ? OR username LIKE ? OR notes LIKE ?", 
                                           (f"%{search_keyword}%", f"%{search_keyword}%", f"%{search_keyword}%"))
        else:
            # 分页查询
            passwords = self.execute_query(f"SELECT id, website, username, password, notes FROM passwords LIMIT ? OFFSET ?", 
        ...
        if search_keyword:
            ...
            # 如果不是搜索模式，总页数基于全部数据
            total_count = self.execute_query("SELECT COUNT(*) FROM passwords")[0][0]
            ...
            if confirmation == QMessageBox.Yes:
                for row in range(self.password_table.rowCount()):
                    ...
                    if checkbox.isChecked():
                        ...
                        self.execute_query("DELETE FROM passwords WHERE id = ?", (password_id,))
                self.load_passwords()
        else:
    ...
    def import_from_excel(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(self, "导入Excel文件", "", "Excel Files (*.xlsx *.xls);;All Files (*)", options=options)
        if file_path:
            try:
                ...
                for index, row in df.iterrows():
                    ...
                    existing_record = self.execute_query("SELECT id FROM passwords WHERE website = ?", (website,))
                    if existing_record:
                        QMessageBox.warning(self, '警告', f'该网址 {website} 已存在，跳过导入。')
                    else:
                        self.execute_query("INSERT INTO passwords (website, username, password, notes) VALUES (?, ?, ?, ?)", 
                                           (website, username, password, notes))
                self.load_passwords()
    ...
    def export_to_excel(self):
        options = QFileDialog.Options()
        file_path, _ = QFileDialog.getSaveFileName(self, "导出Excel文件", "", "Excel Files (*.xlsx);;All Files (*)", options=options)
        if file_path:
            try:
                passwords = self.execute_query("SELECT website, username, password, notes FROM passwords")
                df = pd.DataFrame(passwords, columns=['网站', '用户名', '密码', '备注'])
