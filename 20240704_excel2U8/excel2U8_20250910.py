...
def save_df_to_excel_with_column_width(df, output_file_path, column_widths):
    ...
    # Add license module data
    # Find the next empty row
    next_row = ws.max_row + 2  # Adding 3 rows of space
    
    # Merge cells in the first blank row
    ws.merge_cells(start_row=next_row-1, start_column=1, end_row=next_row-1, end_column=9)
    
    # Add header for license module data
    ws.cell(row=next_row, column=1, value="U8许可模块")
    ws.cell(row=next_row, column=4, value="许可总数")
    ws.cell(row=next_row, column=5, value="已用许可数")
    ws.cell(row=next_row, column=7, value="可用许可数")
    
    # Add license module data rows
    ws.cell(row=next_row+1, column=1, value="人力资源")
    ws.cell(row=next_row+1, column=4, value=13)
    ws.cell(row=next_row+1, column=5, value=13)
    ws.cell(row=next_row+1, column=7, value=0)
    
    ws.cell(row=next_row+2, column=1, value="财务供应链制造")
    ws.cell(row=next_row+2, column=4, value=49)
    ws.cell(row=next_row+2, column=5, value=45)
    ws.cell(row=next_row+2, column=7, value=4)
    ...
    # Set cell alignment to center (for all cells)
    ...
    # Set alignment for license module data to left (after setting general alignment)
    # This ensures it overrides the general center alignment
    license_rows = [next_row, next_row+1, next_row+2]
    license_cols = [1, 4, 5, 8]
    for row in license_rows:
        for col in license_cols:
            cell = ws.cell(row=row, column=col)
            cell.alignment = Alignment(horizontal='left', vertical='center')

    # Add approval signature information
    signature_row = next_row + 3  # Place signature information after license module data
    ws.cell(row=signature_row, column=2, value="统计人：")
    ws.cell(row=signature_row, column=8, value="审批人：")
    ws.cell(row=signature_row+1, column=2, value="日  期：")
    ws.cell(row=signature_row+1, column=8, value="日  期：")
    
    # Set alignment for signature information to left
    signature_cells = [(signature_row, 2), (signature_row, 8), (signature_row+1, 2), (signature_row+1, 8)]
    for row, col in signature_cells:
        cell = ws.cell(row=row, column=col)
        cell.alignment = Alignment(horizontal='left', vertical='center')
