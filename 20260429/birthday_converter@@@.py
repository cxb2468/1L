...
def convert_lunar_to_target_year_solar(lunar_str, target_year):
    ...
    核心逻辑：
    - 尝试用 target_year 和 target_year-1 两个农历年去转换
    - 选择转换后落在 target_year 公历年份的那个结果
    - 例如：农历"十二月初五"在2025年可能对应公历2026-01-23，这就是我们要的
    """
    month, day, is_leap = parse_lunar_date(lunar_str)
    if month is None or day is None:
        return None
    ...
    # 尝试两个农历年份：目标年份和目标年份-1
    # 因为农历年底可能对应公历下一年的年初
    for lunar_year in [target_year, target_year - 1]:
        result = None
        ...
        # 检查转换结果是否在目标公历年份内
        if result is not None and result.year == target_year:
            solar_date_obj = result
            break  # 找到了，跳出循环
    
    if not ZHDATETIME_AVAILABLE and not LUNAR_CALENDAR_AVAILABLE:
        print(f"警告: 没有可用的农历库，无法转换农历日期: {lunar_str}")
    
    return solar_date_obj
    ...
    try:
        ...
        if split_by_month:
            ...
            with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
                ...
                # 设置"2026年公历生日_显示"列的列宽为25
                birthday_display_col = f'{target_year}年公历生日_显示'
                worksheet = writer.sheets['全年汇总']
                set_column_width(worksheet, df_with_month[cols_to_save], birthday_display_col, 25)
                ...
                for month in range(1, 13):
                    ...
                    if len(month_df) > 0:
                        ...
                        set_column_width(worksheet, month_df_clean, birthday_display_col, 25)
